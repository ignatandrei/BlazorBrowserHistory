import { Page,test as baseTest,  Locator, TestInfo } from '@playwright/test';
...
export const test = baseTest.extend<{ page: any }>({
  page: async ({ browser }, use, testInfo) => {
    ...
    context.addInitScript(
          {path:'tests/utils/showMessage.js'   }         
        );
...
export async function flash(button:Locator ): Promise<Locator> {
  console.log("Flashing button before click");
...
export const expect = baseExpect.extend({
  ...
  async toBeVisibleFlash(locator: Locator, options?: { timeout?: number,visible?: boolean  }) {
    const assertionName = 'toBeVisibleFlash';
    let pass: boolean;
